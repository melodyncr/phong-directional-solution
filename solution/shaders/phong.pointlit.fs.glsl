...
void main(void) {
    vec3 lightPosition01 = normalize(uLightPosition - vWorldPosition);
    vec3 worldNormal01 = normalize(vWorldNormal);
    vec3 directionToEye01 = normalize(uCameraPosition - vWorldPosition);
    vec3 reflection01 = 2.0 * dot(worldNormal01, lightPosition01) * worldNormal01 - lightPosition01;

    float lambert = max(dot(worldNormal01, lightPosition01), 0.0);
